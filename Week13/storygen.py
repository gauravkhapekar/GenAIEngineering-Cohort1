...
Generate an engaging opening that introduces the character and sets up the adventure."""
        ...
        try:
            ...
            state.update({
                ...
                "world_state": {"location": "unknown", "time": "day"},
                "mood": "mysterious",
                "current_scene": "opening",
                "story_progression": 1
            })
...
Generate the next scene with 3 meaningful choices for the player."""
        ...
        try:
            ...
            state.update({
                "current_narrative": response,
                "available_choices": [
                    {"id": "continue", "text": "Continue the adventure", "consequences": "unknown"},
                    {"id": "explore", "text": "Explore the area", "consequences": "unknown"},
                    {"id": "rest", "text": "Take a moment to rest", "consequences": "unknown"}
                ],
...
{{
    ...
    "trait_changes": ["new_trait1", "new_trait2"],  // traits to add
...
Determine realistic consequences."""
        ...
        try:
            consequence_data = json.loads(response)
            ...
            state["choices_made"].append({
                "choice_id": choice_id,
                "choice_text": chosen_option["text"],
                "consequence": "Your choice has consequences...",
                "scene": state["current_scene"]
            })
...
{{
    ...
    "story_context": "Updated situation summary"
        ...
        try:
            ...
            pass  # Keep existing world state
...
Make it satisfying and reflect the player's choices throughout the journey."""
        ...
        try:
            ending_data = json.loads(response)
            ending_text = f"""
🎭 **THE END** 🎭
...
- Final Inventory: {', '.join(state['inventory'])}
            ...
            state.update({
                "current_narrative": f"The adventure of {state['character_name']} comes to an end. Through {len(state['choices_made'])} choices, they have become: {', '.join(state['character_traits'])}",
                "available_choices": [],
                "story_complete": True
            })
...
def main():
    ...
    # Start new story
    if new_story or ('story_state' not in st.session_state):
        with st.spinner("🎭 Generating your unique story..."):
            # Generate initial story using the new method
    ...
    if 'story_state' in st.session_state:
                    ...
                    with choice_cols[i]:
                        if st.button(
                            ...
                            with st.spinner("🎲 Processing your choice..."):
                                # Process choice using separate method to avoid recursion
        ...
        with col2:
            ...
            if story_state.get('character_traits'):
                st.markdown("**Traits:**")
                traits_text = " • ".join([f"`{trait}`" for trait in story_state['character_traits']])
